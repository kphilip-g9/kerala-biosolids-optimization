...
FIXED: Delivers to any valid farm to clear STP storage (Aggressive Mode).
"""
...
class ImprovedSolver:
    def __init__(self, state):
        ...
        # FIX: Strip whitespace from columns to ensure matches
    ...
    def _calculate_urgency_score(self, farm_idx: int, stp_idx: int, current_date: datetime) -> float:
        # 1. HARD CONSTRAINT: Yearly Nitrogen Cap
        ...
        # 3. Rain Risk (Simple penalty)
        # If rain > 30mm in next 2 days, penalty.
        if self.state.get_5day_rainfall(farm_idx) > 30.0: return 0.0
            
        # 4. Daily Demand Bonus (Optional)
        farm_id = self.state.farm_locations.iloc[farm_idx]['farm_id']
        if current_date in self.daily_demand.index and farm_id in self.daily_demand.columns:
            if self.daily_demand.loc[current_date, farm_id] > 0:
                score *= 2.0 # Boost priority if they actually need it today
                
        return score

    def allocate_day(self):
        """Run allocation for the current day."""
        ...
        for stp_idx in range(len(self.state.stp_registry)):
            ...
            if available_kg < 5000: continue # Skip if empty
            ...
            for farm_idx in range(len(self.state.farm_locations)):
                ...
                if score > 0:
                    opportunities.append({
                        ...
                        'score': score, 'avail': available_kg
                    })
        ...
        for opp in opportunities:
            ...
            # Send max possible
            max_truck = 20000.0 # 20 Tons
            farm_cap = self.state.farm_n_remaining[farm_idx] / 0.05 # Biosolid mass
...
def run_improved_solver(state):
    print("\n[ImprovedSolver] Starting 365-day simulation (Aggressive)...")
